...
func NewTofu(ctx context.Context,
	...
	*ModuleRuntime, error) {
	return NewTofuWithOptions(ctx, logger, workdir, auxServer, tofuresolver.ResolveOpts{})
}

// NewTofuWithOptions will create a new Tofu client which can be used to
// programmatically interact with the tofu cli
func NewTofuWithOptions(ctx context.Context,
	logger Logger,
	workdir Workdir,
	auxServer *auxprovider.Server,
...
func PickModuleRuntime(
	...
	if strings.HasPrefix(moduleExecutor, "opentofu") || strings.HasPrefix(moduleExecutor, "tofu") {
		...
		return NewTofuWithOptions(ctx, logger, workdir, auxServer, resolveOptions)
	}
